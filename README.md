...
- **v2.7 — true size, not payload size:** the platform truncates the hook's own payload to ~10K chars before Dangi ever sees it, so for file-backed reads Dangi now `stat`s the file **on disk** and reports/tiers on `max(payload size, file size)` instead of the truncated figure. The *trigger* still fires on payload size only — a file that's written but never read still doesn't nudge;
...
- **v2.7 — it remembers:** a nudge on a file-backed blob ≥ 4 KB also records that file as a learned "offender" (see [Detection that learns](#detection-that-learns-v27) below), so the hcat gate can catch it again later even off the static extension list;
